        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRelationships.map((rel, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-blue-500 transition-all duration-300 transform hover:-translate-y-1"
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  >
                    <div className="w-16 h-16 rounded-full bg-gray-700 overflow-hidden mb-2 border-2 border-gray-600 group-hover:border-blue-500 transition-colors">
                      {source?.portraitUrl ? (
                        <img
                    ...
                    <span className="text-white font-semibold text-sm text-center">{source?.name}</span>
                    <span className="text-gray-500 text-xs">{source?.role}</span>
                  </Link>
                  ...
                  <div className="flex flex-col items-center">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-bold text-white ${getRelationshipColor(rel.type)}`}
                      >
                  ...
                  >
                    ...
                    <span className="text-white font-semibold text-sm text-center">{target?.name}</span>
                    <span className="text-gray-500 text-xs">{target?.role}</span>
                  </Link>
                </div>
                <p className="text-gray-400 text-center text-sm">{rel.description}</p>
              </div>
            )
          })}
        </div>
