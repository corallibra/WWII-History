  ...
  if (!character) {
    return (
      <div className="h-full bg-gray-900 flex items-center justify-center px-4">
        <p className="text-white text-xl text-center">{t('character not found')}</p>
      </div>
    )
  }
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6 sm:mb-8">
          ...
          >
            ...
            <span className="text-sm sm:text-base">{t('characters.backToList')}</span>
          </Link>
        </div>

        <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-xl overflow-hidden mb-6 sm:mb-8 border border-gray-700">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0">
            <div className="relative h-48 sm:h-64 md:h-auto bg-gradient-to-br from-gray-700 to-gray-800">
            ...
            <div className="md:col-span-2 p-4 sm:p-6 md:p-8 flex flex-col justify-center">
              <div className="flex items-center gap-3 mb-3 sm:mb-4">
                ...
                <span className="text-gray-400 text-xs sm:text-sm">{character.nationality}</span>
              </div>
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-2">
                {character.name}
              </h1>
              <p className="text-base sm:text-lg md:text-xl text-gray-300">{character.role}</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
          <div className="bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-700">
            ...
            <p className="text-gray-300 text-xs sm:text-sm md:text-base leading-relaxed">{character.biography}</p>
          </div>
          ...
          <div className="bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-700">
            ...
            <ul className="space-y-2">
              {character.achievements.map((achievement, index) => (
                <li key={index} className="text-gray-300 text-xs sm:text-sm flex items-center gap-2">
          ...
          <div className="bg-gray-800 rounded-xl p-4 sm:p-6 border border-gray-700 mb-6 sm:mb-8">
            ...
            <div className="space-y-3 sm:space-y-4">
              {character.quotes.map((quote, index) => (
                <div key={index} className="bg-gray-700/50 rounded-lg p-3 sm:p-4 border-l-3 border-green-500">
                  <p className="text-gray-300 text-xs sm:text-sm italic">"{quote}"</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
              {participatedBattles.map(battle => (
                <Link
                  ...
                  className="bg-gray-700/50 rounded-lg p-3 sm:p-4 border border-gray-700 hover:border-blue-500 transition-colors"
                >
                  <h3 className="text-white font-semibold text-sm">{t(battle.nameKey)}</h3>
            ...
            <div className="space-y-4 sm:space-y-6">
              {character.footprints.map((footprint, index) => (
                <div key={index} className="relative flex gap-4 sm:gap-6">
                  <div className="relative z-10 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-purple-600 flex items-center justify-center text-white font-bold text-xs sm:text-sm">
                    {index + 1}
                  </div>
                  <div className="flex-1 bg-gray-700/50 rounded-lg p-3 sm:p-4 border border-gray-700">
                    <div className="flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-1 sm:gap-4 mb-1 sm:mb-2">
                      ...
                      <span className="text-gray-400 text-xs sm:text-sm">{footprint.date}</span>
                    </div>
                    <p className="text-gray-300 text-xs sm:text-sm">{footprint.description}</p>
                  </div>
                </div>
              ))}
            </div>
